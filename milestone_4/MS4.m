...
acc_GP(k,:) = sum(Ytee==Y_final)/size(Ytee,1);
...
hyp_pca = minimize(hyp, @gp, -40, @infEP, meanfunc, covfunc, likfunc, Xtrr_pca , Ytrr);
[ymu_pca, ys2_pca, fmu_pca, fs2_pca, lp_pca] = gp(hyp_pca, @infEP, meanfunc, covfunc, likfunc, Xtrr_pca, Ytrr, Xtee_pca , Ytee);
% [nlZ dnlZ ] = gp(hyp, @infEP, meanfunc, covfunc, likfunc, Xtr_nl, Ytr);
prob_pca = exp(lp_pca); Y_final_pca = zeros(size(Ytee));
Y_final_pca((prob_pca >= 0.5),:) = 1 ; Y_final_pca((prob_pca < 0.5),:) = -1; 
acc_GP_PCA(k,:) = sum(Ytee==Y_final_pca)/size(Ytee,1);
end
